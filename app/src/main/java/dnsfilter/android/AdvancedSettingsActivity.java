...
public class AdvancedSettingsActivity extends Activity
    ...
    public static void reloadLocalConfig() {
        AdvancedSettingsActivity instance = INSTANCE;
    ...
    private static class MsgTimeoutListener implements TimeoutListener {
        ...
        AdvancedSettingsActivity activity;

        public void setActivity(AdvancedSettingsActivity activity) {
                ...
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (CONFIG.isLocal()) {
                            ...
                            activity.link_field.setMovementMethod(LinkMovementMethod.getInstance());
                        } else {
    ...
    /**
     * ✅ MÉTODO PÚBLICO PARA MOSTRAR POPUP (usado pelo DNSProxyActivity)
     */
    public static void showWelcomePopup(Activity activity) {
        try {
            ...
            if (showInitialInfoPopUp) {
                Dialog popUpDialog = new Dialog(activity, R.style.Theme_dialog_TitleBar);
                    ...
                    public void onClick(View v) {
                        if (disableCheckbox.isChecked()) {
                            try {
                                ...
                                Logger.getLogger().logLine("Erro ao salvar preferência do popup: " + e.toString());
                            }
                        }
                        popUpDialog.dismiss();
                    }
                ...
                int displayWidth = ((WindowManager) activity.getSystemService(WINDOW_SERVICE)).getDefaultDisplay().getWidth();
                int displayHeight = ((WindowManager) activity.getSystemService(WINDOW_SERVICE)).getDefaultDisplay().getHeight();
                window.setLayout((int) (Math.min(displayWidth, displayHeight)*0.9), WindowManager.LayoutParams.WRAP_CONTENT);
                window.setBackgroundDrawableResource(android.R.color.transparent);
            }
        } catch (Exception e) {
            Logger.getLogger().logLine("Erro ao mostrar popup de boas-vindas: " + e.toString());
        }
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        try {
            ...
            mainContentLayout = findViewById(R.id.linearLayout4);
    ...
    private void showPasswordDialog() {
        ...
        if (!dnsManager.isAgentActivated()) {
            Toast.makeText(this, "🔒 Ative o agente primeiro na tela principal", Toast.LENGTH_LONG).show();
            Intent intent = new Intent(this, DNSProxyActivity.class);
        ...
        builder.setTitle("🔐 Acesso às Configurações");
        ...
        builder.setPositiveButton("Entrar", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (enteredPassword.equals(savedPassword)) {
                    ...
                    Toast.makeText(AdvancedSettingsActivity.this, "❌ Senha incorreta!", Toast.LENGTH_SHORT).show();
                    showPasswordDialog();
                }
            }
        });
        ...
        builder.setNegativeButton("Cancelar", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent intent = new Intent(AdvancedSettingsActivity.this, DNSProxyActivity.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
                startActivity(intent);
                finish();
            }
        ...
        AlertDialog passwordDialog = builder.show();
        passwordInput.requestFocus();
    }

    // 🔧 INICIALIZAR UI APÓS AUTENTICAÇÃO
    private void initializeUI() {
        ...
        try {
            ...
            Logger.getLogger().logLine("Error getting debug setting: " + e.toString());
        }
        ...
        if (debug)
            try {
                ...
                Logger.getLogger().logLine("Cannot create logcat file: "+e.toString());
            }

        if (appStart) {
            // ✅ REMOVIDO: Verificação de permissão de notificação - já é feita no DNSProxyActivity
            // ✅ APENAS CARREGAR CONFIGURAÇÃO, NÃO INICIAR SERVIÇO
            loadAndApplyConfig(false);

            // ✅ Mostrar status atual do serviço
    ...
    // ✅ REMOVIDO: onRequestPermissionsResult - as permissões são tratadas no DNSProxyActivity
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (AdvancedFunctions.isServiceRunning()) {
                        ...
                        Logger.getLogger().logLine("📊 Status: " + status);
                        Logger.getLogger().logLine(filterStats);

                        DNSFilterManager dnsManager = DNSFilterManager.getInstance();
                        if (dnsManager != null && dnsManager.isDohEnabled()) {
                            Logger.getLogger().logLine("🔐 DoH está ativo");
                        }
                    } else {
                        Logger.getLogger().logLine("⚠️ Serviço DNS não está rodando");
                        Logger.getLogger().logLine("💡 Inicie o serviço na tela principal");
                    }
                }
            });
        } catch (Exception e) {
            Logger.getLogger().logLine("Erro ao atualizar UI: " + e.toString());
        }
    }
    ...
    public void onBackPressed() {
        if (isAuthenticated) {
            ...
            startActivity(intent);
            finish();
            Logger.getLogger().logLine("📱 Voltando para tela principal - Serviço continua ativo");
        } else {
            Toast.makeText(this, "🔒 Digite a senha para acessar", Toast.LENGTH_SHORT).show();
        }
    }
            ...
            builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (!inputcode.equals(code)) {
                        ...
                        checkPasscode();
                    } else findViewById(R.id.advanced).setVisibility(View.VISIBLE);
                }
            });
    ...
    protected void showFilterRate(boolean asMessage) {
        // ✅ USA AdvancedFunctions
        if (asMessage) {
            AdvancedFunctions.showFilterRate();
        } else {
            Logger.getLogger().logLine(AdvancedFunctions.getFilterStats());
        }
    }
    ...
    protected void handlefilterReload() {
        // ✅ USA AdvancedFunctions
        AdvancedFunctions.reloadFilters();
    }
    ...
    public void onClick(View destination) {
        ...
        if (destination == addFilterBtn) {
            ...
            openBrowser("https://domcustom.com.br/DomCustosAgent/help/help.php");
            return;
        } else if (destination == dnsField) {
            startActivityForResult(new Intent(this, DNSServerConfigActivity.class),
                    DNSServerConfigActivity.ACTIVITY_RESULT_CODE);
            return;
        ...
        // ✅ START - Inicia/Reinicia o serviço
        if (destination == startBtn || destination == enableAdFilterCheck) {
            handleRestart();
        }

        // ⚠️ STOP - Agora com confirmação
        if (destination == stopBtn) {
            //handleStopService(); // Pergunta antes de parar
            // ✅ Voltar ao DNSProxyActivity
            Intent intent = new Intent(this, DNSProxyActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
            startActivity(intent);
            finish();
    ...
    protected synchronized void handleBackToMain() {
        // ⚠️ NUNCA para o serviço - apenas volta para a tela principal

        // Persistir configurações antes de sair
        persistConfig();

        // Limpar sessão de autenticação (OPCIONAL - remova se quiser manter sessão)
        // setAuthenticated(false);

        // Voltar para DNSProxyActivity
        Intent intent = new Intent(this, DNSProxyActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        startActivity(intent);

        Logger.getLogger().logLine("📱 Retornando para tela principal");

        // Finalizar esta activity
        finish();
    }

    protected synchronized void handleStopService() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("⚠️ Parar Serviço DNS?");
        builder.setMessage("Isso desativará a proteção de filtragem DNS.\n\nTem certeza?");

        builder.setPositiveButton("Sim, Parar", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                AdvancedFunctions.stopDNSService(AdvancedSettingsActivity.this);
                Logger.getLogger().logLine("🛑 Serviço DNS parado pelo usuário");
                Toast.makeText(AdvancedSettingsActivity.this, "🛑 Serviço DNS parado", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    protected void handleRestart() {
        if (CONFIG.isLocal()) {
            // ✅ USA AdvancedFunctions para controle de serviço
    ...
    public void onDestroy() {
        ...
        // ⚠️ NÃO parar o serviço aqui - ele deve continuar rodando
        Logger.getLogger().logLine("📱 AdvancedSettingsActivity destruída - Serviço continua ativo");

        // Opcional: limpar autenticação apenas se não for mudança de configuração
        if (!isChangingConfigurations()) {
            // setAuthenticated(false); // Descomente se quiser forçar senha toda vez
        }
    }
